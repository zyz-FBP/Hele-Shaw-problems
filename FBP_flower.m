...
while time <= time_end+dt
   ...
   if is_viz
        ...
        hold on
        plot(sqrt(1+D1^2/2)*cos(theta0),sqrt(1+D1^2/2)*sin(theta0),'r','LineWidth',2,'MarkerSize',5);
        hold on
        plot(curve_old(:,1), curve_old(:,2),'k--','LineWidth',2,'MarkerSize',5);
        hold off
        xlim([-1.5 1.5])
        ylim([-1.5 1.5])
        legend('Initial Boundary','Steady State','Current Boundary','Location','northeast','interpreter','latex','FontSize',10)
...
hold on
quiver(curve0((1:20:end),1),curve0((1:20:end),2),v0(1:20:end,1),v0(1:20:end,2),'k')
hold off
legend('$t=0$','$t=0.02$','$t=0.15$','$V_n\mathbf n(t=0)$','Location','northeast','interpreter','latex','FontSize',14)

figure (3)%compare the max distance and min distance with the radius for steady state
plot([0:100:1.5e04]'*1.0e-05,max_radius(1:100:length(max_radius)),'b','LineWidth',2,'MarkerSize',5);
hold on
plot([0:100:1.5e04]'*1.0e-05,min_radius(1:100:length(min_radius)),'k','LineWidth',2,'MarkerSize',5);
hold on
plot([0:100:1.5e04]'*1.0e-05,ones(151,1)*sqrt(1+D1^2/2),'r--','LineWidth',4,'MarkerSize',5);
hold off
xlim([0 0.15])
xlabel('Time')
ylabel('Distance to the center','interpreter','latex')
legend('maximum distance','minimum distance','accurate radius','FontSize',14)
